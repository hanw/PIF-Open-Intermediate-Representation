...
class HeaderInstance(object):
    ...
    def __init__(self, name, meta_ir_header_attrs, byte_buffer=None, 
                 offset=0, length=None):
        """
        @param name The name of the instance
        @param meta_ir_header_attrs The header attributes from the IR instance. If None,
        ...
        @param length Force the length when meta_ir_header_attrs is None or when length
        is determined externally.
        """

        logging.debug("Adding hdr %s" % name)
        self.name = name
        self.meta_ir_header_attrs = meta_ir_header_attrs
        ...
        if byte_buffer:
            meta_ir_check(isinstance(byte_buffer, bytearray), AirParamError)
        ...
        if not meta_ir_header_attrs: # Opaque block header
            meta_ir_check(length is not None, AirParamError)
            self.length = length
            return

        # @FIXME: 
        # self.is_stack = ("max_depth" in meta_ir_header_attrs.keys() and
        #                  meta_ir_header_attrs["max_depth"] > 1)
        ...
        for field_map in self.meta_ir_header_attrs["fields"]:
...
if __name__ == "__main__":
    ...
    meta_ir_assert(header.length == 14)

    meta_ir_assert(header.get_field("ethertype") == 0, "Failed ethertype get")
    bytes = header.serialize()
    meta_ir_assert(bytes == bytearray(14), "Failed serialize 0 bytes")

    header.set_field("ethertype", 0xffff)
    meta_ir_assert(header.modified, "Header should be modified")
    bytes = header.serialize()
    expected = bytearray(14)
    expected[12] = 0xff
    expected[13] = 0xff
    meta_ir_assert(bytes == expected, "Failed serialize bytes after modify")
