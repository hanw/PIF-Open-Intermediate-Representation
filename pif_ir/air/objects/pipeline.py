...
# A pipeline is initialized with the control flow graph and the AIR 
...
class Pipeline(Processor):
    ...
    @param name The name of the control flow MetaIR object
    @param meta_ir_control_flow_attrs The attributes of the control_flow MetaIR object
    for the pipeline
    @param table_map The map of AIR table objects
    @param action_map The map of AIR action objects
    ...
    * first_table_name Start with this table by default
    """
    def __init__(self, name, meta_ir_control_flow_attrs, table_map, action_map):
        self.name = name
        self.meta_ir_control_flow_attrs = meta_ir_control_flow_attrs
        self.table_map = table_map
        self.action_map = action_map
        self.implementation = meta_ir_control_flow_attrs["implementation"]
        ...
        for edge in self.graph.get_edge_list():
            ...
            if not src_table in self.transitions.keys():
                meta_ir_check(src_table in self.table_map.keys(), AirReferenceError)
        ...
        meta_ir_assert(len(result) == 1, "Control flow has %d entry points; " % 
    ...
    def process(self, parsed_packet):
        ...
        while current_table_name != "exit_control_flow":
            meta_ir_assert(current_table_name in self.transitions.keys(),
...
if __name__ == "__main__":
    ...
    pipe = iri.air_pipeline["ingress_flow"]
