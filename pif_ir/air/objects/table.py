...
class Table(object):
    def __init__(self, name, meta_ir_table_attrs, action_map):
        """
        @brief Constructor for a table object
        @param name The name of the table to instantiate
        @param meta_ir_table_attrs The table attributes from the AIR instance
        @param action_map The map of all action objects for the AIR instance
        ...
        self.name = name
        self.meta_ir_table_attrs = meta_ir_table_attrs
    ...
    def remove_entry(self, entry_ref):
        ...
        if isinstance(entry_ref, table_entry):
            ...
            raise AirReferenceError("Unknown entry ref type for entry remove")
    ...
    def set_default_entry(self, entry):
        meta_ir_assert(isinstance(entry, TableEntryDefault))
        with self.cond_var:
