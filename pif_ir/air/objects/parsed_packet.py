...
class ParsedPacket(object):
    ...
    def __init__(self, original_packet, metadata_dict):
        ...
        meta_ir_check(isinstance(original_packet, bytearray), AirParamError)
    ...
    def parse_header(self, header_name, header_attrs):
        ...
        @param header_attrs The MetaIR attributes associated of the header

        The header is taken from the packet payload and offsets
        are updated.
        """
        meta_ir_assert(header_attrs["type"] in ["header_stack", "header"],
    ...
    def add_header_before(self, header_name, header_attrs, before_header_name):
        ...
        """
        meta_ir_check(before_header_name in self.header_map, MetaIRPacketModException)
        meta_ir_check(header_name not in self.header_map, MetaIRPacketModException)
    ...
    def parse_skip_byte_block(self, bytes):
        ...
        """
        meta_ir_assert(bytes <= self.payload_length,
...
if __name__ == "__main__":
    ...
    from pif_ir.air.instance import AirInstance
    def transmit_packet(out_port, packet):
        pass

    # Create config from unit_test
    local_dir = os.path.dirname(os.path.abspath(__file__))
    iri = AirInstance("instance", local_dir + "/../tests/unit_test.yml",
                      transmit_packet)

    # Create 100 byte packet
    byte_buf = bytearray(range(100))
    ppkt = ParsedPacket(byte_buf, {})
    meta_ir_assert(ppkt.payload_length == 100, "Failed parsed pkt length check")
    meta_ir_assert(ppkt.payload_offset == 0, "Failed parsed pkt length check")
    meta_ir_assert(ppkt.header_length == 0, "Header length not 0 on init")
    
    # Replicate the packet
    repl = ppkt.replicate()
    meta_ir_assert(repl.payload_length == 100, "Failed repl pkt length check")
    meta_ir_assert(repl.payload_offset == 0, "Failed repl pkt length check")
    meta_ir_assert(repl.header_length == 0, "Repl header length not 0 on init")

    # Parse an ethernet header; verify packet updated, replicant not.
    ppkt.parse_header("ethernet", iri.meta_ir_object_map["ethernet"])
    meta_ir_assert(ppkt.payload_length == 100 - 14, "Parsed pkt length check")
    meta_ir_assert(ppkt.payload_offset == 14, "Parsed pkt offset check")
    meta_ir_assert(ppkt.header_length == 14, "Header len not 14 after eth add")
    
    meta_ir_assert(repl.payload_length == 100, "Repl pkt length check")
    meta_ir_assert(repl.payload_offset == 0, "Repl pkt offset check")
    meta_ir_assert(repl.header_length == 0, "Repl header len not 0 after eth add")

    # Check set/get unparsed fields
    meta_ir_assert(ppkt.get_field("ipv4.version") == None,
               "Unparsed field get should return None")
    meta_ir_assert(ppkt.set_field("ipv4.version", 3) == None,
               "Unparsed field set should return None")

    # Check get field ops
    exp_eth_val = 0x0c0d # Expected value; 
    eth_val = ppkt.get_field("ethernet.ethertype")
    meta_ir_assert(eth_val == exp_eth_val, 
    ...
    meta_ir_assert(mac_val == exp_mac_val, 
    ...
    # Add a header
    meta_ir_assert(ppkt.add_header_after("ethernet", iri.header["ethernet"],
                                     "ethernet") is None,
               "Should fail to add existing header ethernet")
    ip_hdr_len = ppkt.add_header_after("ipv4", iri.header["ipv4"], "ethernet")
    meta_ir_assert(ip_hdr_len == 20, "Adding ipv4, expected len %d. got %d" % 
               (20, ip_hdr_len))

    # Should not have affected the payload
    meta_ir_assert(ppkt.payload_length == 100 - 14, "Parsed pkt length check")
    meta_ir_assert(ppkt.payload_offset == 14, "Parsed pkt offset check")
    meta_ir_assert(ppkt.header_length == 14 + ip_hdr_len, "hdr len check")

    hdr_len = ppkt.remove_header("ethernet")
    meta_ir_assert(hdr_len == 14, "Remove should return header length")

    # Should not have affected the payload
    meta_ir_assert(ppkt.payload_length == 100 - 14, "Post remove length check")
    meta_ir_assert(ppkt.payload_offset == 14, "Post remove offset check")
    meta_ir_assert(ppkt.header_length == ip_hdr_len, "Post remove hdr len")
