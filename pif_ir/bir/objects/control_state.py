...
class ControlState(object):
    ...
    def _get_offset(self, packet, bit_offset):
        for cond in self.offset:
            if isinstance(cond, str) or isinstance(cond, int):
                return self.bir_parser.eval_inst(cond, self.header, packet,
                                                 bit_offset)
            elif self.bir_parser.eval_cond(cond[0], self.header, packet,
                                           bit_offset):
                return self.bir_parser.eval_inst(cond[1], self.header, packet,
                                                 bit_offset)
        raise BIRError("didn't find offset!")

    def _get_basic_block(self, packet, bit_offset):
    ...
    def process(self, packet, bit_offset):
        offset = self._get_offset(packet, bit_offset)
        basic_block = self._get_basic_block(packet, bit_offset)
        if basic_block == "$done$":
            basic_block = None
