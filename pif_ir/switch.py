...
#
# @file
# @brief Thread instance for AIR switch
#
# The Switch class inherits from Thread. It takes a dataplane object
# and a MetaIR configuration reference (filename, for example). It loads
# the MetaIR configuration as an AIR instance. Then it polls the dataplane
# for packets, passing them to the AIR instance.
...
class Switch(Thread):
    ...
    """
    def __init__(self, IRinstance, name, input, dataplane):
        """
        @param name The name of the switch instance
        @param input A file or file name with the MetaIR YAML for the switch
        ...
        self.killed = False
        self.instance = IRinstance(name, input, dataplane.send)
