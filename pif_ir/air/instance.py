...
#
# @file
# @brief The top level AIR configuration object
#
...
class AirInstance(MetaIRInstance):
    """
    @brief An Intermediate Representation configuration instance

    @param air_parser A map from parser name to AIR parser object
    @param air_action A map from action name to AIR action object
    @param air_table A map from table name to AIR table object
    @param air_pipeline A map from control flow name to a pipeline object. 
    This combines control flow with tables and actions
    @param air_traffic_manager A map from TM name to TM object
    ...
    An IR instance extends the MetaIR instance and additionally instantiates
    AIR objects with the parameters indicated.

    The AIR instance is also responsible for the layout of the processor
    ...
    def __init__(self, name, input, transmit_handler):
        ...
        @todo Add support to allow the specification of the MetaIR instance
        """
...
if __name__ == "__main__":
    ...
    local_dir = os.path.dirname(os.path.abspath(__file__))
    obj = AirInstance("instance", local_dir + "/tests/unit_test.yml",
                      transmit_packet)
