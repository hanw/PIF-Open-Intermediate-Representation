...
class AirPrimitiveModifyField(PrimitiveAction):
    def __init__(self, args):
        self.name = "ModifyField"
        meta_ir_assert(len(args) in [2, 3], "Bad param list for modify_field")
...
class AirPrimitiveAddHeader(PrimitiveAction):
    def __init__(self, args):
        self.name = "AddHeader"
        meta_ir_assert(len(args) == 1, "Bad param list for add_header")
...
class AirPrimitiveRemoveHeader(PrimitiveAction):
    def __init__(self, args):
        self.name = "RemoveHeader"
        meta_ir_assert(len(args) == 1, "Bad param list for remove_header")
        self.header_ref = args[0]
        # Verify header_ref is valid reference
        logging.debug("Prim action %s. Hdr %s" % (self.name, self.header_ref))
...
class AirPrimitiveAddToField(PrimitiveAction):
    def __init__(self, args):
        self.name = "AddToField"
        meta_ir_assert(len(args) == 2, "Bad param list for add_to_field")
...
class AirPrimitiveNoOp(PrimitiveAction):
    def __init__(self, args):
        self.name = "NoOp"
        meta_ir_assert(len(args) == 0, "Bad param list for no_op")
...
primitive_action_to_class = {
    "modify_field"   : AirPrimitiveModifyField,
    "add_header"     : AirPrimitiveAddHeader,
    "remove_header"  : AirPrimitiveRemoveHeader,
    "add_to_field"   : AirPrimitiveAddToField,
    "no_op"          : AirPrimitiveNoOp,
}
...
class Action(object):
    ...
    """
    def __init__(self, name, meta_ir_action_attrs):
        self.name = name
        if "parameter_list" in meta_ir_action_attrs.keys():
            self.param_list = meta_ir_action_attrs["parameter_list"]
        else:
            self.param_list = {}
        self.primitives = []
        self.param_refs = set()
        ...
        prim_calls = meta_ir_action_attrs["implementation"].split(";")[:-1]
        for call in prim_calls:
            call = call.replace("\n","").strip()
            prim_name, args = call.split("(", 2)
            meta_ir_assert(prim_name in primitive_action_to_class.keys(),
    ...
    def eval(self, parsed_packet, action_params):
        ...
        meta_ir_assert(set(action_params.keys()) == set(self.param_list),
