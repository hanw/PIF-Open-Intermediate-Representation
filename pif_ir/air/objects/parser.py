    ...
    def _add_edge(self, edge):
        ...
        if "in_value_set" in attrs:
            ...
            if not set_name in self.all_value_sets.keys():
                raise AirReferenceError("Parser: unknown value set %s" 
                                            % set_name)
...
class Parser(Processor):
    ...
    @param meta_ir_parser_attrs The attributes from the MetaIR description
    ...
    @todo Should we just pass the AIR instance object in for init?
    """
    def __init__(self, name, meta_ir_parser_attrs, all_parser_states,
        ...
        self.all_value_sets = all_value_sets
        self.meta_ir_parser_attrs = meta_ir_parser_attrs
        self.implementation = meta_ir_parser_attrs["implementation"]
        ...
        self.first_state_name = meta_ir_parser_attrs["start_state"]
    ...
    def process(self, parsed_packet, state=None):
        ...
        meta_ir_check(isinstance(parsed_packet, ParsedPacket), AirParamError)
...
if __name__ == "__main__":
    ...
    # Create AIR config from unit_test
    if len(sys.argv) > 2:
        unit_test_input = sys.argv[2:]
    else:
        local_dir = os.path.dirname(os.path.abspath(__file__))
        unit_test_input = local_dir + "/../tests/unit_test.yml"
    logging.info("Using input file %s" % unit_test_input)
    iri = AirInstance("instance", unit_test_input, transmit_packet)

    parser = iri.air_parser["parser"]
    ...
    if "ethernet" in iri.header.keys():
        meta_ir_assert(ppkt.header_length == 14, "Did not parser ether hdr")
        meta_ir_assert("ethernet" in ppkt.header_map.keys(),
                   "Did not parser ether hdr")
        meta_ir_assert(ppkt.get_field("ethernet"),
                   "Failed ethernet header valid check")
        meta_ir_assert(not ppkt.get_field("foo"),
    ...
    if "ethernet" in iri.header.keys():
        ...
        if "vlan_tag_outer" in iri.header.keys():
            meta_ir_assert(ppkt.header_length == 18, "Did not parser ether+vlan hdr")
            meta_ir_assert("vlan_tag_outer" in ppkt.header_map.keys(), 
